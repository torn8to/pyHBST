...
// ds Node template specialization for descriptors of type:
// CDescriptorBinaryProbabilistic
template <typename ProbabilisticMatchableType_,
          typename real_precision_ = double>
class ProbabilisticNode
    : public BinaryNode<ProbabilisticMatchableType_, real_precision_> {
  // ds readability
 ...
 public:
  ...
  // of attribute initialization is left to the subclass ds the default
  // constructor is required, since we do not want to trigger the automatic leaf
  // spawning of the baseclass in a subclass
  ProbabilisticNode() {}
  ...
  // ds access
 public:
  // ds implementing the new spawn leafs function to handle the modified
  // descriptors
  virtual const bool spawnLeafs(const SplittingStrategy& train_mode_) override {
    ...
    if (1 < number_of_matchables) {
      ...
      BitStatisticsVector bit_probabilities_accumulated(
          BitStatisticsVector::Zero());
      ...
      // ds get average
      const BitStatisticsVector bit_probabilities_mean(
          bit_probabilities_accumulated / number_of_matchables);

      // ds compute variance
      for (uint32_t index_bit = 0; index_bit < Matchable::descriptor_size_bits;
           ++index_bit) {
        ...
        if (this->bit_mask[index_bit]) {
          ...
          for (const Matchable* matchable : this->matchables) {
            ...
            const real_precision_ delta =
                matchable->bit_probabilities[index_bit] -
                bit_probabilities_mean[index_bit];
      ...
      if (-1 != this->index_split_bit) {
        ...
        this->partitioning = std::fabs(
            0.5 - this->_getSetBitFraction(this->index_split_bit,
                                           this->matchables,
                                           this->number_of_on_bits_total));
        ...
        if (0 < this->number_of_on_bits_total && 0.5 > this->partitioning) {
          ...
          matchables_leaf_zeroes.reserve(number_of_matchables -
                                         this->number_of_on_bits_total);
          ...
          this->right = new Node(
              this->depth + 1, matchables_leaf_ones, mask, train_mode_);

          assert(0 < matchables_leaf_zeroes.size());
          this->left = new Node(
              this->depth + 1, matchables_leaf_zeroes, mask, train_mode_);
