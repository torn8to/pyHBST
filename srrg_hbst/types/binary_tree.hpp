...
class BinaryTree {
 ...
 public:
  ...
  using MatchVectorMap = std::unordered_map<uint64_t, MatchVector>;
  using MatchVectorMapElement = std::pair<uint64_t, MatchVector>;

#ifdef SRRG_MERGE_DESCRIPTORS
  //! @brief component object used for matchable merging
  struct MatchableMerge {
    MatchableMerge(const Matchable* query_,
                   ObjectType query_object_,
                   Matchable* reference_)
        : query(query_), query_object(query_object_), reference(reference_) {}
    const Matchable* query;   // to be absorbed matchable
    ObjectType query_object;  // affected object that might have to update its
                              // matchable reference
...
#endif
    static constexpr size_t descriptor_size_bits =
        Matchable::descriptor_size_bits;
  };
 ...
 public:
  ...
  BinaryTree(const uint64_t& identifier_)
      : _header(identifier_), _root(nullptr) {
    _matchables.clear();
  ...
  BinaryTree(
      ...
      : _header(identifier_), _root(new Node(matchables_, train_mode_)) {
    _matchables.clear();
    _matchables.insert(
        _matchables.end(), matchables_.begin(), matchables_.end());
    _matchables_to_train.clear();
    _added_identifiers_train.clear();
  ...
  // ds free all nodes in the tree without freeing the matchables - call
  // clear(true)
  ~BinaryTree() { clear(); }

  // ds shared pointer access wrappers
 public:
  const uint64_t getNumberOfMatches(
      const std::shared_ptr<const MatchableVector> matchables_query_,
      const uint32_t& maximum_distance_ = 25) const {
    return getNumberOfMatches(*matchables_query_, maximum_distance_);
  }

  const typename Node::real_type getMatchingRatio(
      const std::shared_ptr<const MatchableVector> matchables_query_,
      const uint32_t& maximum_distance_ = 25) const {
    return getMatchingRatio(*matchables_query_, maximum_distance_);
  }

  const uint64_t getNumberOfMatchesLazy(
      const std::shared_ptr<const MatchableVector> matchables_query_,
      const uint32_t& maximum_distance_ = 25) const {
    return getNumberOfMatchesLazy(*matchables_query_, maximum_distance_);
  }

  const typename Node::real_type getMatchingRatioLazy(
      const std::shared_ptr<const MatchableVector> matchables_query_,
      const uint32_t& maximum_distance_ = 25) const {
  ...
  const typename Node::real_type getMatchingRatioLazy(
      const MatchableVector& matchables_query_,
      const uint32_t& maximum_distance_ = 25) const {
  ...
  //! @brief returns database size (i.e. the number of added images/matchable
  //! vectors with unique identifiers)
  //! @returns number of added reference matchable vectors
  const size_t size() const {
    assert(_header.number_of_training_entries ==
           _added_identifiers_train.size());
  ...
  //! @brief returns the number of matchables added for training (before
  //! compression)
  ...
  //! @brief returns the actual number of stored matchable objects (after
  //! compression) this number is always equal to the uncompressed one if
  //! SRRG_MERGE_DESCRIPTORS is disabled
  //! @returns number of matchables
...
#ifdef SRRG_MERGE_DESCRIPTORS
  //! @brief matchable merges from last match/add/train call - intended for
  //! external bookkeeping update only
  //! @returns a vector of effective merges between matchables (recall that the
  //! memory of Mergable.query is already freed)
    ...
    for (const Matchable* matchable_query : matchables_query_) {
      ...
      while (node_current) {
        ...
        if (node_current->has_leafs) {
          ...
          for (const Matchable* matchable_reference :
               node_current->matchables) {
            if (maximum_distance_ >
                matchable_query->distance(matchable_reference)) {
              ++number_of_matches;
              break;
            }
          }
  ...
  const ScoreVector getScorePerImage(
    ...
    // ds identifier to vector index mapping - simultaneously initialize result
    // vector
    ...
    for (const uint64_t& identifier_reference : _added_identifiers_train) {
      scores_per_image[mapping_identifier_image_to_score.size()]
          .identifier_reference = identifier_reference;
      mapping_identifier_image_to_score.insert(std::make_pair(
          identifier_reference, mapping_identifier_image_to_score.size()));
    }
    ...
    for (const Matchable* matchable_query : matchables_query_) {
      ...
      while (node_current) {
        ...
        if (node_current->has_leafs) {
          ...
          // ds check current descriptors for each reference image in this node
          // and exit
          std::set<uint64_t> matched_references;
          for (const Matchable* matchable_reference :
               node_current->matchables) {
            if (matchable_query->distance(matchable_reference) <
                maximum_distance_) {
#ifdef SRRG_MERGE_DESCRIPTORS
              for (const ObjectMapElement& object :
                   matchable_reference->objects) {
                const uint64_t& identifier_reference = object.first;
#else
              const uint64_t& identifier_reference =
                  matchable_reference->_image_identifier;
#endif

                // ds the query matchable can be matched only once to each
                // reference image
                if (matched_references.count(identifier_reference) == 0) {
                  ++scores_per_image[mapping_identifier_image_to_score.at(
    ...
    for (Score& score : scores_per_image) {
      score.matching_ratio =
          score.number_of_matches / number_of_query_descriptors;
    }

    // ds if desired, sort in descending order by matching ratio
    if (sort_output) {
      std::sort(scores_per_image.begin(),
                scores_per_image.end(),
    ...
    for (const Matchable* matchable_query : matchables_query_) {
      ...
      while (node_current) {
        ...
        if (node_current->has_leafs) {
          ...
          // ds check current descriptors in this node and exit
          if (maximum_distance_ >
              matchable_query->distance(node_current->matchables.front())) {
            ++number_of_matches;
          }
          break;
        }
  ...
  // ds direct matching function on this tree
  template <class T>
  MatchVector matchWrapper(const std::vector<ObjectType>& keypoints,
    ...
    getMatchablesFromPythonInput<T>(
        keypoints, descriptors, image_identifier, matchables);
  ...
  const std::shared_ptr<const MatchVector> getMatchesLazy(
      const std::shared_ptr<const MatchableVector> matchables_query_,
      const uint32_t& maximum_distance_ = 25) const {
    // ds wrap the call
    MatchVector matches;
  ...
  //! @param[out] matches_ output matching results: contains all available
  //! matches for all training images added to the tree
  //! @param[in] maximum_distance_ the maximum distance allowed for a positive
  //! match response
  void match(const MatchableVector& matchables_query_,
  ...
  //! @param[in] matchables_ new input matchables to integrate into the current
  //! tree (transferring the ownership!)
  ...
  void add(
    ...
    assert(matchables_.front()->_image_identifier ==
           matchables_.back()->_image_identifier);
    ...
    // ds train based on set matchables (no effect for do
    // SplittingStrategy::DoNothing)
    train(train_mode_);
  }

  //! @brief train tree with current _trainable_matchables according to selected
  //! mode
  //! @param[in] train_mode_ desired training mode
  void train(
      const SplittingStrategy& train_mode_ = SplittingStrategy::SplitEven) {
    if (_matchables_to_train.empty() ||
        train_mode_ == SplittingStrategy::DoNothing) {
      return;
    }
    _header.number_of_matchables_uncompressed += _matchables_to_train.size();

    // ds check if we have to build an initial tree first (no training
    // afterwards)
    if (!_root) {
      _root = new Node(_matchables_to_train, train_mode_);
      assert(_matchables.empty());
      _matchables.insert(_matchables.end(),
                         _matchables_to_train.begin(),
                         _matchables_to_train.end());
...
#ifdef SRRG_MERGE_DESCRIPTORS
    // ds we need delayed insertion as we continuously scan the current
    // references for merging
    ...
    // ds for each new descriptor - buffering new matchables and merging
    // identical ones
      ...
      while (node_current) {
        ...
        if (node_current->has_leafs) {
          ...
          for (const Matchable* matchable_reference :
            ...
            // ds and this reference has not absorbed a matchable already in
            // this call
          ...
          // ds if insertion is required - we could not merge the query
          // matchable
          if (insertion_required) {
            // ds bookkeep matchable for addition
            // ds we need to bookkeep the node in order to add it to its
            // matchables later on
    ...
    for (MatchableMerge& mergable : _merged_matchables) {
      ...
      // ds free query (!) recall that the tree takes ownership of the
      // matchables
  ...
  //! @param[in] matchables_ query matchables, which will also automatically be
  //! added to the tree (transferring the ownership!)
  //! @param[out] matches_ output matching results: contains all available
  //! matches for all training images added to the tree
  //! @param[in] maximum_distance_matching_ the maximum distance allowed for a
  ...
  void matchAndAdd(
    ...
    const uint64_t identifier_image_query =
        matchables_.front()->_image_identifier;
    ...
    for (Matchable* matchable_query : matchables_) {
      ...
      while (node_current) {
        ...
        if (node_current->has_leafs) {
          ...
          // ds obtain best matches in the current leaf via brute-force search -
          // bookkeeping matches to merge (distance == 0)
          std::map<uint64_t, Match> best_matches;
#ifdef SRRG_MERGE_DESCRIPTORS
          ...
          if (matchable_reference &&
            ...
            _merged_matchables.emplace_back(
                MatchableMerge(matchable_query,
                               std::move(matchable_query->_object),
                               matchable_reference));
            merged_reference_matchables.insert(matchable_reference);
          } else {
#endif
    ...
    _matchables.insert(
        _matchables.end(), new_matchables.begin(), new_matchables.end());
  ...
  MatchVectorMap matchAndAddWrapper(
    ...
    MatchableVector matchables;
    getMatchablesFromPythonInput(
        keypoints, descriptors, image_identifier, matchables);
    MatchVectorMap matches_per_reference_image;
    matchAndAdd(matchables,
                matches_per_reference_image,
  ...
  // ds creates a matchable vector (pointers) from opencv descriptors - only
  // available if OpenCV is present on building system
  static const MatchableVector getMatchables(
      const cv::Mat& descriptors_cv_,
    ...
    for (int64_t index_descriptor = 0; index_descriptor < descriptors_cv_.rows;
         ++index_descriptor) {
      matchables[index_descriptor] =
          new Matchable(objects_[index_descriptor],
                        descriptors_cv_.row(index_descriptor),
                        identifier_tree_);
    }
    return matchables;
  }
  ...
  bool write(const std::string& file_path) const {
    ...
    if (!outfile.is_open()) {
      std::cerr << "BinaryTree::write|ERROR: unable to open file: " << file_path
                << std::endl;
      return false;
    }
    if (!outfile.good()) {
      std::cerr << "BinaryTree::write|ERROR: file is not valid: " << file_path
                << std::endl;
      return false;
    }
    ...
    GUARDED_IO(outfile,
               ...
               endianness_check,
               1,
               "BinaryTree::write|ERROR: unable to write");

    // ds populate leaf count in header (currently only used for
    // de/serialization)
    ...
    for (const Node* leaf : leafs) {
      ...
      for (auto it = indices_split_bit.rbegin(); it != indices_split_bit.rend();
           ++it) {
        GUARDED_IO(outfile,
                   write,
  ...
  bool read(const std::string& file_path) {
    ...
    if (!infile.is_open()) {
      std::cerr << "BinaryTree::read|ERROR: unable to open file: " << file_path
                << std::endl;
      return false;
    }
    if (!infile.good()) {
      std::cerr << "BinaryTree::read|ERROR: file is not valid: " << file_path
                << std::endl;
      return false;
    }
    ...
    GUARDED_IO(infile,
               ...
               endianness_check,
               1,
               "BinaryTree::read|ERROR: unable to read endian byte");
    if (endianness_check[0] != char(0)) {
      std::cerr << "BinaryTree::read|ERROR: invalid endianness, database saved "
                   "on different arch"
                << std::endl;
      infile.close();
      return false;
    ...
    GUARDED_IO(
        infile, read, reinterpret_cast<char*>(&_header), sizeof(_header), "");
    for (size_t i = 0; i < _header.number_of_training_entries; ++i) {
      uint64_t identifier = 0;
      GUARDED_IO(infile,
                 read,
                 reinterpret_cast<char*>(&identifier),
                 sizeof(identifier),
                 "");
      _added_identifiers_train.insert(identifier);
    }
    assert(_added_identifiers_train.size() ==
           _header.number_of_training_entries);

    // ds leaf buffer (static to allow easy escapes without requiring
    // deallocation) ds TODO use more compact data structure(s)
    ...
    for (size_t i = 0; i < _header.number_of_leafs; ++i) {
      ...
      // ds read split bit indices order - note that we also have to read the -1
      // of the leaf
      ...
      objects_per_descriptor.reserve(
          leaf_header.number_of_matchables_compressed);
      for (size_t j = 0; j < leaf_header.number_of_matchables_compressed; ++j) {
        ...
        ObjectMap objects;
        for (size_t index_object = 0; index_object < number_of_objects;
             ++index_object) {
          uint64_t key = 0;
          ObjectType object;
          GUARDED_IO(infile,
    ...
    if (number_of_read_matchables != _header.number_of_matchables_compressed) {
      std::cerr << "BinaryTree::read|ERROR: number of loaded matchables "
                   "inconsistent with header"
                << std::endl;
      return false;
    }

    // ds after this point we use dynamic memory to build the tree - no
    // exceptions are thrown! ds assemble actual database by evaluating all
    // leafs
    _root = new Node();
    ...
    for (size_t i = 0; i < leaf_headers.size(); ++i) {
      ...
      const std::vector<ObjectMap>& objects_per_descriptor =
          objects_per_descriptor_per_leaf[i];

      // ds grab any descriptor for evaluation decision rule
      // ds this is fine since all the descriptors reside in the same leaf and
      // satisfy that rule
      ...
      while (current) {
        ...
        if (bit_index_order[current->_header.depth] == -1) {
          ...
          assert(descriptors.size() ==
                 leaf_header.number_of_matchables_compressed);

          // ds populate matchables of the leaf
          assert(current->matchables.empty());
          current->matchables.reserve(descriptors.size());
          for (size_t index_descriptor = 0;
               index_descriptor < descriptors.size();
               ++index_descriptor) {
            current->matchables.emplace_back(
                new Matchable(objects_per_descriptor[index_descriptor],
                              descriptors[index_descriptor]));
          }
          current->_header = std::move(leaf_header);
          _matchables.insert(_matchables.end(),
                             current->matchables.begin(),
                             current->matchables.end());
          break;
        } else {
        ...
        if (current->index_split_bit !=
            bit_index_order[current->_header.depth]) {
    ...
    if (_matchables.size() != _header.number_of_matchables_compressed) {
      std::cerr << "BinaryTree::read|ERROR: unable to reconstruct tree with "
                   "read matchables "
 ...
 protected:
#ifdef SRRG_MERGE_DESCRIPTORS
  //! @brief retrieves best matches (BF search) for provided matchables for all
  //! image indices
  //! @param[in] matchable_query_
  //! @param[in] matchables_reference_
  //! @param[in] maximum_distance_matching_
  //! @param[in,out] best_matches_ best match search storage: image id, match
  //! candidate
  void _matchExhaustive(const Matchable* matchable_query_,
    ...
    ObjectType object_query = std::move(
        matchable_query_->objects.at(matchable_query_->_image_identifier));
    ...
    for (const Matchable* matchable_reference : matchables_reference_) {
      ...
      if (distance < maximum_distance_matching_) {
        ...
        for (const ObjectMapElement& object : matchable_reference->objects) {
          ...
          try {
            // ds update match if current is better than the current best - will
            // jump to addition if no best is available
            Match& best_match_so_far =
                best_matches_.at(identifer_tree_reference);
            ...
            if (distance < best_distance_so_far) {
              ...
              // ds replace the best with this match on the spot - we don't have
              // to update the query information
              best_match_so_far.matchable_references = std::move(
                  std::vector<const Matchable*>(1, matchable_reference));
            ...
            // ds if the match is equal to the last (multiple candidates)
              ...
              // ds add the candidate - we don't have to update the distance
              // since its the same as the best
              best_match_so_far.matchable_references.push_back(
                  matchable_reference);
            ...
            // ds add a new match
            best_matches_.insert(std::make_pair(identifer_tree_reference,
                                                Match(matchable_query_,
                                                      matchable_reference,
                                                      object_query,
                                                      object.second,
                                                      distance)));
  ...
  //! @param[in,out] best_matches_ best match search storage: image id, match
  //! candidate
  //! @param[in,out] matchable_reference_for_merge_ reference matchable with
  //! distance == 0 (matchable merge candidate)
  void _matchExhaustive(const Matchable* matchable_query_,
                        const MatchableVector& matchables_reference_,
                        const uint32_t& maximum_distance_matching_,
    ...
    for (const Matchable* matchable_reference : matchables_reference_) {
      ...
      if (distance < maximum_distance_matching_) {
        ...
        // ds if the matchable descriptors are identical - we can merge - note
        // that maximum_distance_for_merge must always be smaller than
        // maximum_distance_matching_
        assert(maximum_distance_for_merge < maximum_distance_matching_);
        if (distance <= maximum_distance_for_merge) {
          // ds behold the power of C++ (we want to keep the MatchableVector
          // elements const)
          matchable_reference_for_merge_ =
              const_cast<Matchable*>(matchable_reference);
        }
      }
    }
  ...
  void _matchExhaustive(const Matchable* matchable_query_,
    ...
    for (const Matchable* matchable_reference : matchables_reference_) {
      ...
      // ds if matching distance is within the threshold
      if (distance < maximum_distance_matching_) {
        const uint64_t& identifer_tree_reference =
            matchable_reference->_image_identifier;
        ...
        ObjectType object_reference = std::move(
        ...
        try {
          ...
          // ds if the match is equal to the last (multiple candidates)
            ...
            // ds add the candidate - we don't have to update the distance since
            // its the same as the best
            best_match_so_far.matchable_references.push_back(
                matchable_reference);
  ...
  //! @brief recursively counts all leafs and descriptors stored in the tree
  //! (expensive)
  ...
  void getMatchablesFromPythonInput(
    ...
    for (size_t i = 0; i < keypoints.size(); ++i) {
      matchables[i] =
          new Matchable(keypoints[i], descriptors[i], image_identifier);
    }
  }
 ...
 protected:
  ...
  //! @brief bookkeeping: trainable matchables resulting from last matchAndAdd
  //! call
  std::vector<Trainable> _trainables;

#ifdef SRRG_MERGE_DESCRIPTORS
  //! @brief bookkeeping: merged matchable pairs (query -> reference) resulting
  //! from last matchAndAdd call over Mergable.query one has access to the
  //! merged (=freed) matchable and can update external bookkeeping accordingly
