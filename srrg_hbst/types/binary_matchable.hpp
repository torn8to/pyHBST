...
class BinaryMatchable {
 ...
 public:
  ...
  //! @brief descriptor size in bytes (for all matchables, size after the number
  //! of augmented bits)
  static constexpr uint32_t raw_descriptor_size_bytes =
      descriptor_size_bits_ / 8;

  //! @brief descriptor size, bits in whole bytes (corresponds to
  //! descriptor_size_bits for a byte-wise descriptor)
  static constexpr uint32_t descriptor_size_bits_in_bytes =
      raw_descriptor_size_bytes * 8;
 ...
 public:
  ...
  //! @param[in] object_ associated object (note that HBST takes ownership of
  //! this object and invalidates the variable)
  //! @param[in] descriptor_ HBST descriptor
  //! @param[in] image_identifier_ reference to image on which the descriptors
  //! have been computed (optional)
...
// ds wrapped constructors - only available if OpenCV is present on building
// system ds this choice is slower since we have to perform a conversion:
// getDescriptor()
#ifdef SRRG_HBST_HAS_OPENCV
  ...
  BinaryMatchable(ObjectType object_,
                  ...
                  const uint64_t& image_identifier_ = 0)
      : BinaryMatchable(
            object_, getDescriptor(descriptor_), image_identifier_) {}
#endif

  //! @brief default destructor
  virtual ~BinaryMatchable() { objects.clear(); }

  // ds functionality
 public:
  //! @brief computes the classic Hamming descriptor distance between this and
  //! another matchable
  //! @param[in] matchable_query_ the matchable to compare this against
  //! @returns the matching distance as integer
  inline const uint32_t distance(
      const BinaryMatchable<ObjectType_, descriptor_size_bits_>*
          matchable_query_) const {
    return (matchable_query_->descriptor ^ this->descriptor).count();
  }

#ifdef SRRG_MERGE_DESCRIPTORS
  //! @brief merges a matchable with THIS matchable (desirable when having to
  //! store identical descriptors)
  //! @param[in] matchable_ the matchable to merge with THIS
  inline void merge(
      const BinaryMatchable<ObjectType_, descriptor_size_bits_>* matchable_) {
  ...
  //! @brief merges a matchable with THIS matchable (desirable when having to
  //! store identical descriptors)
  //! @brief this method has been created for quick matchable merging where
  //! matchable_ only contains a single entry for identifier and pointer
  //! @param[in] matchable_ the matchable to merge with THIS
  inline void mergeSingle(
      const BinaryMatchable<ObjectType_, descriptor_size_bits_>* matchable_) {
    objects.insert(std::make_pair(matchable_->_image_identifier,
                                  std::move(matchable_->_object)));
    ++number_of_objects;
    assert(number_of_objects == objects.size());
  }
#endif
  ...
  //! @brief enables manual update of all linked objects (without changing the
...
#ifdef SRRG_HBST_HAS_OPENCV
  //! @brief descriptor wrapping - only available if OpenCV is present on
  //! building system
  ...
  static inline Descriptor getDescriptor(const cv::Mat& descriptor_cv_) {
    ...
    for (uint64_t byte_index = 0; byte_index < raw_descriptor_size_bytes;
         ++byte_index) {
      const uint32_t bit_index_start = byte_index * 8;

      // ds grab a byte and convert it to a bitset so we can access the single
      // bits
      const std::bitset<8> descriptor_byte(
          descriptor_cv_.at<uchar>(byte_index));
    ...
    if (descriptor_size_bits_overflow > 0) {
      ...
      const std::bitset<8> descriptor_byte(
          descriptor_cv_.at<uchar>(raw_descriptor_size_bytes));
  ...
  //#ifdef PYTHON_BUILD
  static inline Descriptor getDescriptorFromBoolVector(
      const std::vector<bool>& bool_descriptor) {
    // PRBABLY NOT THE BEST WAY TO DO THAT. FIND A BETTER WAY FOR BITSET AND
    // PYBIND11 ds buffer
    Descriptor binary_descriptor(descriptor_size_bits_);
    ...
    // std::copy(bool_descriptor.begin(), bool_descriptor.end(),
    // std::ostream_iterator<bool>(strStream));
  ...
  static inline Descriptor getDescriptorFromUint8Vector(
      const std::vector<uint8_t>& uint8_descriptor) {
    // ds buffer
    Descriptor binary_descriptor(descriptor_size_bits_);
    // ds set original descriptor string after augmentation
    for (uint64_t byte_index = 0; byte_index < raw_descriptor_size_bytes;
    ...
    if (descriptor_size_bits_overflow > 0) {
      // ds get last byte (not fully set)
      const std::bitset<8> descriptor_byte(
          uint8_descriptor[raw_descriptor_size_bytes]);
 ...
 public:
  ...
  //! @brief a connected object correspondences - when using this field one must
  //! ensure the permanence of the referenced object!
  ObjectMap objects;

  //! @brief quick access to the number of contained objects/image_identifiers
  //! (default: 1)
 ...
 protected:
  //! @brief single value access only: linked object to group of descriptors
  //! (e.g. an image or image index)
  const uint64_t _image_identifier;

  //! @brief single value access only: linked object to descriptor (e.g.
  //! keypoint or an index)
  ObjectType _object;
...
constexpr uint32_t
    BinaryMatchable<ObjectType_, descriptor_size_bits_>::descriptor_size_bits;
template <typename ObjectType_, uint32_t descriptor_size_bits_>
constexpr uint32_t
    BinaryMatchable<ObjectType_,
                    descriptor_size_bits_>::raw_descriptor_size_bytes;
template <typename ObjectType_, uint32_t descriptor_size_bits_>
constexpr uint32_t
    BinaryMatchable<ObjectType_,
                    descriptor_size_bits_>::descriptor_size_bits_in_bytes;
template <typename ObjectType_, uint32_t descriptor_size_bits_>
constexpr uint32_t
    BinaryMatchable<ObjectType_,
                    descriptor_size_bits_>::descriptor_size_bits_overflow;
