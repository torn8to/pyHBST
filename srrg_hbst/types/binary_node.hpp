...
class BinaryNode {
 ...
 public:
  ...
  // ds the default constructor is triggered by subclasses - the responsibility
  // of attribute initialization is left to the subclass ds this is required,
  // since we do not want to trigger the automatic leaf spawning of the
  // baseclass in a subclass
  BinaryNode() {}
 ...
 public:
  ...
  virtual const bool spawnLeafs(const SplittingStrategy& train_mode_) {
    ...
    switch (train_mode_) {
      case SplittingStrategy::SplitEven: {
        ...
        for (uint32_t bit_index = 0;
             bit_index < Matchable::descriptor_size_bits;
             ++bit_index) {
          ...
          if (bit_mask[bit_index]) {
            ...
            const double partitioning_current =
                std::fabs(0.5 - _getSetBitFraction(
                                    bit_index, matchables, number_of_set_bits));
      ...
      case SplittingStrategy::SplitRandomUniform: {
        ...
        if (available_bits.size() > 0) {
          std::uniform_int_distribution<uint32_t> available_indices(
              0, available_bits.size() - 1);

          // ds sample uniformly at random
          index_split_bit =
              available_bits[available_indices(Node::random_number_generator)];

          // ds compute distance for this index (0.0 is perfect)
          partitioning = std::fabs(
              0.5 - _getSetBitFraction(
                        index_split_bit, matchables, number_of_on_bits_total));
        }
        break;
      }
      default: {
    ...
    // ds if best was found and the partitioning is sufficient (0 to 0.5) - we
    // can spawn leaves
    if (index_split_bit != -1 && partitioning < maximum_partitioning) {
      ...
      // ds first we have to split the descriptors by the found index -
      // preallocate vectors since we know how many ones we have
      MatchableVector matchables_ones(number_of_on_bits_total);
      MatchableVector matchables_zeros(matchables.size() -
                                       number_of_on_bits_total);

      // ds loop over all descriptors and assigning them to the new vectors
      // based on bit status
 ...
 public:
  ...
  const uint64_t& getDepth() const { return _header.depth; }
 ...
 protected:
  ...
  BinaryNode(Node* parent_,
    ...
    // ds recompute current number of contained merged matchables TODO make this
    // less horribly wasteful
 ...
 public:
  ...
  //! @brief maximum achieved descriptor group partitioning using the
  //! index_split_bit
  static real_type maximum_partitioning;

  //! @brief maximum tree depth (leaf spawning blocks if reached, default:
...
template <typename BinaryMatchableType_, typename real_type_>
real_type_ BinaryNode<BinaryMatchableType_, real_type_>::maximum_partitioning =
    0.1;
template <typename BinaryMatchableType_, typename real_type_>
...
template <typename BinaryMatchableType_, typename real_type_>
std::mt19937
    BinaryNode<BinaryMatchableType_, real_type_>::random_number_generator;
